...
function initHostsInfiniteScroll($, current_subnet, address_add_url, address_change_url, ip_uuid) {
    ...
    var renderedPages = 5,
        fetchedPages = [],
        busy = false,
        nextPageUrl = '/api/open_ipam/subnet/' + current_subnet + '/hosts/',
        searchQuery = '',
    ...
    function validateIp(ip_address, callback) {
        ...
        $.ajax({
            type: 'GET',
            url: '/api/open_ipam/subnet/' + current_subnet + '/hosts/?start=' + ip_address,
        ...
        validateIp(input, function (isValid) {
            if (isValid) {
                ...
                if (input !== searchQuery) {
                    searchQuery = input;
                    nextPageUrl = '/api/open_ipam/subnet/' + current_subnet + '/hosts/?start=' + searchQuery;
                    $('#subnet-visual').empty();
                    fetchedPages = [];
                    lastRenderedPage = 0;
                    busy = false;
